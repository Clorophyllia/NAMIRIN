...
//static TH1D *hADCCARYAHists_raw[N_ADC_arya_silicon];
static TH1D *hADCCARYAHists_cal[N_ADC_arya_silicon];
static TH1D *hADCCARYAHists_ped[N_ADC_arya_silicon];
static TH1D *hADCCARYAHists_ped1[N_ADC_arya_silicon];
static TH1D *hADCCARYAHists_ped2[N_ADC_arya_silicon];
//static TH2D *hP2DADCARYAHists_raw;
static TH1D *hSUMADCARYAHists_raw;

static TH2D *hP2DADCARYAHists_cal;
static TH2D *hP2DADCARYAHists_ped;
float pedestal_arya[N_ADC_arya_silicon];
extern float y_inter_indiv[N_ADC_arya_silicon+1], slope_indiv[N_ADC_arya_silicon+1];//f3 test config
//float y_inter_indiv[N_ADC_arya_silicon+1], slope_indiv[N_ADC_arya_silicon+1];//f3 test config
...
/*
   ...
   char titlecal[256];*/
/*
   for (int i=0; i<N_ADC_arya_silicon; i++)
   {
       //       printf("%0x\n",id);d
       //TF1 *f1 = new TF1("f1","gaus",0,200);
       sprintf(name,  "F3SILICONARYA-%03d", i);
       sprintf(title, "F3SILICONARYA-%03d", i);
      // hADCCARYAHists_raw[i] = h1_book<TH1D>(name, title, F3SILICONARYA_N_BINS, F3SILICONARYA_X_LOW, F3SILICONARYA_X_HIGH);
       sprintf(nameped,  "F3SILICONARYAPED-%03d", i);
       sprintf(nameped1,  "F3SILICOARYANPED1-%03d", i);
       sprintf(nameped2,  "F3SILICOARYANPED2-%03d", i);	
       sprintf(titleped, "F3SILICONARYAPED-%03d", i);
       sprintf(namecal,  "F3SILICONARYACAL-%03d", i);
       sprintf(titlecal, "F3SILICONARYACAL-%03d", i);
       //hADCCARYAHists_ped[i] = h1_book<TH1D>(nameped, titleped, F3SILICONARYA_N_BINS, F3SILICONARYA_X_LOW, F3SILICONARYA_X_HIGH);
       //hADCCARYAHists_ped1[i] = h1_book<TH1D>(nameped1, titleped1, F3SILICONARYA_N_BINS, F3SILICONARYA_X_LOW, F3SILICONARYA_X_HIGH);
       //hADCCARYAHists_ped2[i] = h1_book<TH1D>(nameped2, titleped2, F3SILICONARYA_N_BINS, F3SILICONARYA_X_LOW, F3SILICONARYA_X_HIGH);	
       //hADCCARYAHists_cal[i] = h1_book<TH1D>(namecal, titlecal, F3SILICONARYA_N_BINS, F3SILICONARYA_X_LOW, F3SILICONARYA_X_HIGH);
       //f1_ped_fit[i] = f1_book<TF1>(name, title, F3SILICON_N_BINS, F3SILICON_X_LOW, F3SILICON_X_HIGH);
   }
//   hP2DADCARYAHists_raw = h2_book<TH2D>("F3 silicon ARYA 2D", "F3 silicon ARYA 2D plot", 40, 0, 40, 100, 0, 4100);
   hSUMADCARYAHists_raw = h1_book<TH1D>("silicon ARYA sum all channels", "silicon ARYA all channel sum", F3SILICONARYA_N_BINS, F3SILICONARYA_X_LOW, F3SILICONARYA_X_HIGH);
   hP2DADCARYAHists_ped = h2_book<TH2D>("F3 silicon ARYA 2D pedestal", "F3 silicon ARYA 2D plot pedestal", 40, 0, 40, 100, 0, 4100);
   hP2DADCARYAHists_cal = h2_book<TH2D>("F3 silicon ARYA 2D calibrated", "F3 silicon ARYA 2D plot calibrated", 40, 0, 40, 100, 0, 4100);

   first_Call_chk_arya=1;
	...
	if (myfile.is_open())
	{
		while (!myfile.eof())
			...
			pedestal_arya[line_ch] = line_ped; 
		    	//printf( "pedestal ch, ped= %d %f \n",line_ch, line_ped);
			printf( "pedestal ch, ped = %d %f\n",line_ch, pedestal_arya[line_ch]);
		   }
	    myfile.close();
...
/*	myfile.open (pedestalname);
	...
	myfile.close();*/
/*
	float mean_gd[N_ADC_arya_silicon];
  	float sd_gd[N_ADC_arya_silicon];
	float mean_gd_avg = 0.;
	float mean_am[N_ADC_arya_silicon];
  	float sd_am[N_ADC_arya_silicon];
	float mean_am_avg = 0.;
...
/*
	...
	for (int i=15; i<N_ADC_f3_silicon; i++) {
    	//hADCCARYAHists_ped[i]->Fit(f1,"R");
    	//hADCCARYAHists_ped[i]->Fit(f2,"R+");
...
INT sili_arya_ADC(EVENT_HEADER *pheader, void *pevent)
{
 //  INT i;
/*   DWORD *pdata;
   DWORD *pdata_cal;
