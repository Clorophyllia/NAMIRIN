...
static TH1D *hADCCHists_cal[N_ADC_f3_silicon];
static TH1D *hADCCHists_ped[N_ADC_f3_silicon];
static TH1D *hADCCHists_ped1[N_ADC_f3_silicon];
static TH1D *hADCCHists_ped2[N_ADC_f3_silicon];
static TH2D *hP2DADCHists_raw;
static TH1D *hSUMADCHists_raw;

static TH2D *hP2DADCHists_cal;
static TH2D *hP2DADCHists_ped;
*/
...
/*
   for (int i=0; i<N_ADC_f3_silicon; i++)
   {
       ...
       sprintf(nameped,  "F3SILICONPED-%03d", i);
       sprintf(nameped1,  "F3SILICONPED1-%03d", i);
       sprintf(nameped2,  "F3SILICONPED2-%03d", i);	
       sprintf(titleped, "F3SILICONPED-%03d", i);
       sprintf(namecal,  "F3SILICONCAL-%03d", i);
       sprintf(titlecal, "F3SILICONCAL-%03d", i);
       hADCCHists_ped[i] = h1_book<TH1D>(nameped, titleped, F3SILICON_N_BINS, F3SILICON_X_LOW, F3SILICON_X_HIGH);
       hADCCHists_ped1[i] = h1_book<TH1D>(nameped1, titleped1, F3SILICON_N_BINS, F3SILICON_X_LOW, F3SILICON_X_HIGH);
       hADCCHists_ped2[i] = h1_book<TH1D>(nameped2, titleped2, F3SILICON_N_BINS, F3SILICON_X_LOW, F3SILICON_X_HIGH);	
       hADCCHists_cal[i] = h1_book<TH1D>(namecal, titlecal, F3SILICON_N_BINS, F3SILICON_X_LOW, F3SILICON_X_HIGH);
       //f1_ped_fit[i] = f1_book<TF1>(name, title, F3SILICON_N_BINS, F3SILICON_X_LOW, F3SILICON_X_HIGH);
   }
   hP2DADCHists_raw = h2_book<TH2D>("F3 silicon 2D", "F3 silicon 2D plot", 40, 0, 40, 100, 0, 4100);
   hSUMADCHists_raw = h1_book<TH1D>("silicon sum all channels", "silicon all channel sum", F3SILICON_N_BINS, F3SILICON_X_LOW, F3SILICON_X_HIGH);

   hP2DADCHists_ped = h2_book<TH2D>("F3 silicon 2D pedestal", "F3 silicon 2D plot pedestal", 40, 0, 40, 100, 0, 4100);
   hP2DADCHists_cal = h2_book<TH2D>("F3 silicon 2D calibrated", "F3 silicon 2D plot calibrated", 40, 0, 40, 100, 0, 4100);
...
/*	myfile.open (pedestalname);
	...
	myfile.close();*/
/*
	float mean_gd[N_ADC_f3_silicon];
  	float sd_gd[N_ADC_f3_silicon];
	float mean_gd_avg = 0.;
	float mean_am[N_ADC_f3_silicon];
  	float sd_am[N_ADC_f3_silicon];
	float mean_am_avg = 0.;
	...
	for (int i=15; i<N_ADC_f3_silicon; i++) {
    	hADCCHists_ped[i]->Fit(f1,"R");
    	hADCCHists_ped[i]->Fit(f2,"R+");
