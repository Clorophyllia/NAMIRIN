...
INT f2_ppac_begin(INT run_number, char *error, TRIGGER_SETTINGS *tsr){
/*
	INT16 window_width=tsr->f2_ppac_window_width/25;

	INT16 window_offset=0;
	if (tsr->f2_ppac_window_offset>0 || tsr->f2_ppac_window_offset==0){window_offset=tsr->f2_ppac_window_offset/25;}
	if (tsr->f2_ppac_window_offset<0){window_offset=4096-(tsr->f2_ppac_window_offset/25)*-1;}

	INT16 extra_search_margin=tsr->f2_ppac_extra_search_margin/25;
	INT16 reject_margin=tsr->f2_ppac_reject_margin/25;
	INT blt_event_num=1;
...
//	addr = (F2_PPAC_ADDR<<16)|CVT_V1190_STATUS_ADD;
//	CAENVME_ReadCycle(BHandle, addr, &module_status, CVT_V1190_STATUS_AM, CVT_V1190_STATUS_DATA_SIZE);
	//if(module_status&0x0004) printf("module Status: %x \n",module_status);
...
INT f2_ppac_read_event(int32_t BHandle, const char *bank_name, char *pevent, INT off, uint32_t *buff, int buff_size, uint32_t *pdata, int bkm){
			...
			case CVT_V1190_TDC_MEASURE:
					{
						...
						*pdata++=channel;
						if(channel == 0) *pdata++= vir_tdc_tof[0];
						else *pdata++=measure;
